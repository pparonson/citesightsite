...
import { matchFilter, nip19 as nip192 } from "nostr-tools";
...
var NDKRelay = class extends EventEmitter2 {
  ...
  async publishEvent(event, timeoutMs) {
    ...
    const publish = this.relay.publish(nostrEvent);
    let publishTimeout;
...
var NDKRelaySet = class {
  ...
  async publish(event, timeoutMs) {
    ...
    const promises = Array.from(this.relays).map(
      (relay) => {
...
var NDKSubscription2 = class extends EventEmitter3 {
  subId;
  filters;
  opts;
  relaySet;
...
var NDKSubscriptionGroup = class extends NDKSubscription2 {
  subscriptions;
  ...
  isEventForSubscription(event, subscription) {
    ...
    if (!filters)
      return false;
    return matchFilter(filters[0], event.rawEvent());
  }
      ...
      if (Array.isArray(value)) {
        if (result[key] === void 0) {
          ...
          result[key] = Array.from(
            /* @__PURE__ */ new Set([...result[key], ...value])
          );
        }
      } else {
        result[key] = value;
...
function relaysFromBech32(bech322) {
  try {
    const decoded = nip192.decode(bech322);
  ...
  if (contactListEvents) {
    ...
    contactListEvents.forEach((event) => {
      event.tags.forEach((tag) => {
        if (tag[0] === "p") {
          try {
            const npub = nip193.npubEncode(tag[1]);
            npubs.add(npub);
          } catch (e) {
          }
        }
...
var NDKUser = class {
  ...
  constructor(opts) {
    if (opts.npub)
      this.npub = opts.npub;
    if (opts.hexpubkey) {
      this.npub = nip194.npubEncode(opts.hexpubkey);
    }
    if (opts.relayUrls) {
   ...
   */
  hexpubkey() {
    return nip194.decode(this.npub).data;
  }
  /**
...
function encode() {
  if (this.isParamReplaceable()) {
    ...
    return nip195.neventEncode({
      id: this.tagId(),
      relays: [this.relay.url],
      author: this.pubkey
    });
  } else {
    return nip195.noteEncode(this.tagId());
  }
}
...
var NDKPool = class extends EventEmitter5 {
  ...
  addRelay(relay, connect = true) {
    ...
    relay.on(
      "notice",
      (relay2, notice) => this.emit("notice", relay2, notice)
    );
...
var NDKList = class extends NDKEvent {
  ...
  get name() {
    ...
    if (this.kind === 1e4 /* MuteList */ && !nameTag) {
      return "Mute";
...
// src/index.ts
var NDK8 = class extends EventEmitter7 {
  pool;
  signer;
  cacheAdapter;
  debug;
  ...
  subscribe(filters, opts, relaySet, autoStart = true) {
    const subscription = new NDKSubscription2(this, filters, opts, relaySet);
    if (relaySet) {
...
export {
  ...
  NDKRelayStatus,
  NDKRepost,
  NDKSubscription2 as NDKSubscription,
  NDKSubscriptionCacheUsage,
